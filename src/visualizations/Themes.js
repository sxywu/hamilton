...
var Themes = React.createClass({
  ...
  updateThemes() {
    ...
    this.themes = this.container.selectAll('h3')
      .data(groupedThemes, d => d.name);

    this.themes.exit().remove();

    var enter = this.themes.enter().append('h3')
    ...
    // diamond groups
    var diamonds = svgs.selectAll('g')
      .data(d => d.diamonds, d => d.id);
    ...
    diamonds.select('path')
      .attr('d', this.calculateCurve);

    diamonds.select('text')
      .attr('x', d => d.x2)
